...
Walker* walker_create(
    Maze* maze,
    int start_cell_pos[2],
    enum Direction start_dir,
    void (*pos_callback)(float pos[3]),
    void (*rot_callback)(float rot[3]),
    void (*fin_callback)()
) {
...
static int cell_passage_in_direction(Cell* cell, enum Direction dir) {
  switch (dir) {
  case UP: return cell->up; break;

  case DOWN: return cell->down; break;

  case LEFT: return cell->left; break;

  case RIGHT: return cell->right; break;
  }
  return 0;
}
...
static void create_new_interpolation(Walker* walker) {
  ...
  if (cell_passage_in_direction(cur_cell, rotate_direction_right(walker->direction)))
    walk_right(walker); // first look right
  else if (cell_passage_in_direction(cur_cell, walker->direction))
    walk_straight(walker); // then forward
  else if (cell_passage_in_direction(cur_cell, rotate_direction_left(walker->direction)))
    walk_left(walker); // then left
  else
    walk_turn(walker); // else turn right (180 degrees)
}

static float pan_from_direction(enum Direction dir) {
  switch (dir) {
  case UP: return 0.0; break;

  case DOWN: return 180.0; break;

  case LEFT: return 270.0; break;

  case RIGHT: return 90.0; break;
  }
  return 0.0;
}
...
static void get_global_position(Walker* walker, float pos[3]) {
  ...
  case UP: pos[2] += 0.5; break;

  case DOWN: pos[2] -= 0.5; break;

  case LEFT: pos[0] += 0.5; break;

  case RIGHT: pos[0] -= 0.5; break;
  }
}

static void walk_straight(Walker* walker) {
  switch (walker->direction) {
  case UP: walker->cell[1]--; break;

  case DOWN: walker->cell[1]++; break;

  case RIGHT: walker->cell[0]++; break;

  case LEFT: walker->cell[0]--; break;
  }
