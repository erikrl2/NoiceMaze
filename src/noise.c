...
void noise_generate_texture2d_channel(int gridsize, int width, int height, int stride, float* data) {
  ...
  for (i = 0; i < gridsize; i++) noise_grid[i] = (float*)malloc(gridsize * sizeof(float));

  for (i = 0; i < gridsize; i++)
    for (j = 0; j < gridsize; j++) noise_grid[i][j] = rand() / (float)RAND_MAX;
  ...
  for (i = 0; i < gridsize; i++) free(noise_grid[i]);
  free(noise_grid);
}
