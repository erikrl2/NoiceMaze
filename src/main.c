...
static enum screensaverParameter get_screensaver_parameter(int argc, const char* argv[]) {
  if (argc == 1)
    ...
    if (arg[0] == '-' || arg[0] == '/') {
      ...
      case 'c': return CONFIGURATION;
      case 'p': return PREVIEW; // Preview in Settings screen
      case 's': return FULLSCREEN; // When user clicks Preview button in Settings screen
      }
    }
  }
