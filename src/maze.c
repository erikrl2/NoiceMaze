...
void maze_generate_cell(Maze* maze, Cell* cell) {
  ...
  while (1) {
    directions = 0;
    if (cell->y != 0)
      if (maze_cell_isfree(maze, cell->x, cell->y - 1)) paths[directions++] = UP;
    if (cell->y != maze->height - 1)
      if (maze_cell_isfree(maze, cell->x, cell->y + 1)) paths[directions++] = DOWN;
    if (cell->x != 0)
      if (maze_cell_isfree(maze, cell->x - 1, cell->y)) paths[directions++] = LEFT;
    if (cell->x != maze->width - 1)
      if (maze_cell_isfree(maze, cell->x + 1, cell->y)) paths[directions++] = RIGHT;
  ...
  for (y = 0; y < maze->height; y++) {
    ...
    for (x = 0; x < maze->width; x++) {
      ...
      if (cell->object == OBJ_TWISTER)
        strcat(buf, "T");
      else if (cell->object == OBJ_START)
        strcat(buf, "S");
      else if (cell->object == OBJ_FINISH)
        strcat(buf, "F");
      else
        strcat(buf, " ");
      if (cell->right)
