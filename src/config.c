...
static void set_value_auto_free(char* name, char* value); // sets a config value, does not copy the strings, frees
                                                          // parameter-strings when they are not used
...
void config_load() {
  ...
  while (fgets(str, 128, file) != NULL) {
    ...
    set_value_auto_free(
        ...
        string_create_from(str + symbol_index + 1, strlen(str) - symbol_index - 2)
    );
  }
}
...
void config_print() {
  ...
  for (config = config_first; config != NULL; config = config->next) printf("%s=%s\n", config->name, config->value);
}
